...
void Image::printMatrix(double** matrix, int len1, int len2)
{
	for (int i = 0; i < len1; i++)
	{
		for (int j = 0; j < len2; j++)
			cout << setprecision(5) << matrix[i][j] << endl;
		cout << "----------------" << endl;
	}
}
	...
	for (int i = 0; i < height; i++)
		for (int j = 0; j < width; j++)
			for (int k = 0; k < layers; k++)
				...
				image3D_D[i][j][0] = abs(image3D_D[i][j][0]);
...
double** Image::getFeatures15D(double **features, int size)
{
...
{
	unsigned char* classes = allocate_1D(size, 1, 1);
...
void Image::imgSegment(Image result, int wndsize, int num, bool flag3D)
{
	...
	int pad_width = imgPaddingDouble(5);
	//Copy the Data
	double*** temp = allocate_3D_Double(width+2*pad_width, height+2*pad_width, bpp);
	for (int i = 0; i < height+2*pad_width; i++)
		for (int j = 0; j < width+2*pad_width; j++)
			temp[i][j][0] = image3D_Extended_Double[i][j][0];
	//Image Features Initialization
	...
	for (int f = 0; f < 25; f++)
	{
		...
		//cout << image3D_Double[440][597][0] << endl;

		//Get Positive Values
		absoluteValues(image3D_Double, 1);
		//STEP2 : Use Mean to Average the Energies
		int pad_width1 = imgPaddingDouble(wndsize);
		//cout << pad_width1 << endl;
		for (int i = pad_width1; i < height + pad_width1; i++)
			for (int j = pad_width1; j < width + pad_width1; j++)
			{
				...
				img_feature25D[i-pad_width1][j-pad_width1][f] = sum / (wndsize * wndsize);
			}
	}

	//STEP3 : Get 15 Dimensional Features
	for (int i = 0; i < height; i++)
		for (int j = 0; j < width; j++)
		{
			...
			for (int k = 0; k < 15; k++)
				img_feature15D[i][j][k] = img_feature15D[i][j][k] / img_feature25D[i][j][0];
		}
	//STEP4 : Get 14 Dimensional Features
	for (int i = 0; i < height; i++)
		for (int j = 0; j < width; j++)
			for (int k = 0; k < 14; k++)
				img_feature14D[i*width+j][k] = img_feature15D[i][j][k+1];

	//STEP5 : Get 3 Dimensional Features
	double** img_feature3D = allocate_2D_Double(3, height*width);
	img_feature3D = PCA(img_feature14D, height*width, 14);

	//STEP5 : K-Means Segmentation
	...
	for (int i = 0; i < height; i++)
		for (int j = 0; j < width; j++)
		{
			...
			if (classes[temp] == 0)
			{
				...
				result.image3D[i][j][1] = 0;
				result.image3D[i][j][2] = 0;
			}
			if (classes[temp] == 1)
			{
				result.image3D[i][j][0] = 0;
				result.image3D[i][j][1] = 255;
				result.image3D[i][j][2] = 0;
			}
			if (classes[temp] == 2)
			{
...
//K-means Get Codebook
double** Image::KmeansCenter(double **features, int num, int size, int dimension)
{
	unsigned char* classes = allocate_1D(size, 1, 1);
	double* histo = allocate_1D_Double(num, 1, 1);
	double** center = allocate_2D_Double(dimension+1, num);
	//Initialize with first n positions
	int r[num];
	srand(time(0));
	for (int i = 0; i < num; i++)
...
//SIFT
double** Image::imgSift(Image another, int KeyNum, int num, bool showflag, int flag)
{
	//Read the Images
	Mat imgSource = cv::imread(fileName, cv::IMREAD_GRAYSCALE);
	Mat imgSecond = cv::imread(another.fileName, cv::IMREAD_GRAYSCALE);
	Mat imgSourceColor = cv::imread(fileName, cv::IMREAD_COLOR);
	Mat imgSecondColor = cv::imread(another.fileName, cv::IMREAD_COLOR);

	//Show the Image in Order
	if (showflag)
	{
		namedWindow("Source Image", WINDOW_AUTOSIZE);
		namedWindow("Second Image", WINDOW_AUTOSIZE);
		imshow("Source Image", imgSource);
		imshow("Second Image", imgSecond);
		waitKey(0);
	}

	//Extract the KeyPoints
	int KeyPtsNum = KeyNum;
	Mat discriptorSource;
	Mat discriptorSecond;
	vector<KeyPoint> KeyPtsSrc;
	vector<KeyPoint> KeyPtsSnd;
	SIFT sift = SIFT(KeyPtsNum, 3, 0.03, 10, 1.6);
	sift.operator()(imgSource, noArray(), KeyPtsSrc, discriptorSource, false);
	sift.operator()(imgSecond, noArray(), KeyPtsSnd, discriptorSecond, false);
	//cout << discriptorSource.rows << endl;
	//cout << discriptorSecond.rows << endl;

	//Draw Keypoints In the Image
	if (showflag)
	{
		Mat keypointSourceImage;
		Mat keypointSecondImage;
		drawKeypoints(imgSource, KeyPtsSrc, keypointSourceImage, Scalar::all(-1), DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
		drawKeypoints(imgSecond, KeyPtsSnd, keypointSecondImage, Scalar::all(-1), DrawMatchesFlags::DRAW_RICH_KEYPOINTS);

		//Show Key Points in Images
		namedWindow("Key Points of Source Image", WINDOW_AUTOSIZE);
		namedWindow("Key Points of Second Image", WINDOW_AUTOSIZE);
		imshow("Key Points of Source Image", keypointSourceImage);
		imshow("Key Points of Second Image", keypointSecondImage);
		imwrite("Keypoints.jpg", keypointSourceImage);
		waitKey(0);
		...
		matcher.match(discriptorSource, discriptorSecond, matchPoints, Mat());
		drawMatches(imgSourceColor, KeyPtsSrc, imgSecondColor, KeyPtsSnd, matchPoints, imageOutput);
		namedWindow("Match Points", WINDOW_AUTOSIZE);
		imshow("Match Points",imageOutput);
		imwrite("Match.jpg", imageOutput);
		waitKey(0);

		//Find the Largest Keypoint
		int largest_index = LargestKeypoint(discriptorSource, KeyPtsNum);
		cout << largest_index << endl;
		cout << "Size of Largest Keypoints: " << KeyPtsSrc[largest_index].size << endl;
		cout << "Angle of Largest Keypoints: " << KeyPtsSrc[largest_index].angle << endl;
		cout << "Response of Largest Keypoints: " << KeyPtsSrc[largest_index].response << endl;
		cout << "Octave of Largest Keypoints: " << KeyPtsSrc[largest_index].octave << endl;

		//Draw Largest Keypoint
		vector<KeyPoint> KeyPtsSrcMax;
		KeyPtsSrcMax.assign(KeyPtsSrc.begin()+largest_index, KeyPtsSrc.begin()+largest_index+1);
		drawKeypoints(imgSourceColor, KeyPtsSrcMax, keypointSourceImage, Scalar::all(-1), DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
		namedWindow("Largest Key Points of Source Image", WINDOW_AUTOSIZE);
		imshow("Largest Key Points of Source Image", keypointSourceImage);
		imwrite("Largest.jpg", keypointSourceImage);
		waitKey(0);

		//Find the Nearest Keypoint
		int nearest_index = NearestKeyPoint(discriptorSource, discriptorSecond, largest_index, KeyPtsNum);
		cout << nearest_index << endl;
		cout << "Size of Nearest Keypoints: " << KeyPtsSnd[nearest_index].size << endl;
		cout << "Angle of Nearest Keypoints: " << KeyPtsSnd[nearest_index].angle << endl;
		cout << "Response of Nearest Keypoints: " << KeyPtsSnd[nearest_index].response << endl;
		cout << "Octave of Nearest Keypoints: " << KeyPtsSnd[nearest_index].octave << endl;

		//Draw Nearest Keypoint
		vector<KeyPoint> KeyPtsSndMax;
		KeyPtsSndMax.assign(KeyPtsSnd.begin()+nearest_index, KeyPtsSnd.begin()+nearest_index+1);
		drawKeypoints(imgSecondColor, KeyPtsSndMax, keypointSecondImage, Scalar::all(-1), DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
		namedWindow("Largest Key Points of Second Image", WINDOW_AUTOSIZE);
		imshow("Largest Key Points of Second Image", keypointSecondImage);
		imwrite("Largest_Second.jpg", keypointSecondImage);
		waitKey(0);
	}

	//Get the CodeBooks
	double** codebook = allocate_2D_Double(128, KeyPtsNum);
	for (int i = 0; i < KeyPtsNum; i++)
	{
		float* Mi = discriptorSource.ptr<float>(i);
		for (int j = 0; j < 128; j++)
			codebook[i][j] = (double)Mi[j];
	}
	
	//K-Means centers
	double** centers = allocate_2D_Double(129, num);
	centers = KmeansCenter(codebook, num, KeyPtsNum, 128);
	if (flag)
		return centers;
	return codebook;
}
...
int Image::LargestKeypoint(Mat discriptor, int KeyPtsNum)
{
	int index = 0;
	double maxscale = 0;
	for (int i = 0; i < KeyPtsNum; i++)
	{
		double dist = 0;
		float* Mi = discriptor.ptr<float>(i);
		for (int j = 0; j < 128; j++)
			//cout << Mi[j] << endl;
			dist += pow(Mi[j], 2);
		dist = sqrt(dist);
...
int Image::NearestKeyPoint(Mat discriptor1, Mat discriptor2, int largest_index, int KeyPtsNum)
{
	int index = 0;
	double mindist = 9999;
	for (int i = 0; i < KeyPtsNum; i++)
	{
		...
		double len1 = 0;
		double len2 = 0;
		float* M1 = discriptor1.ptr<float>(largest_index);
		float* M2 = discriptor2.ptr<float>(i);
		for (int j = 0; j < 128; j++)
		{
			dist_euc += pow((M2[j] - M1[j]), 2);
			dist_cos += M2[j] * M1[j];
			len1 += M1[j] * M1[j];
			len2 += M2[j] * M2[j];
		}
		dist_cos = dist_cos / (sqrt(len1) * sqrt(len2));
...
//Sort the Centroids
double** Image::sortCentroids(double** centroid, double** target, int num, int dimension)
{
	int order[num];
	double dist[num];
	double* tempCenter = allocate_1D_Double(dimension+1, 1, 1);
	...
	for (int i = 0; i < num; i++)
	{
		...
		for (int j = 0; j < num; j++)
		{
			double tempDist = 0;
			for (int k = 0; k < dimension; k ++)
				tempDist += pow(centroid[i][k] - target[j][k], 2);
	...
	for (int i = 0; i < num-1; i++)
		for (int j = i + 1; j < num-1; j++)
			if (order[i] > order[j])
			{
				...
				order[i] = order[j];
				order[j] = temp;
				//Swap Centroids Array
				tempCenter = centroid[i];
				centroid[i] = centroid[j];
				centroid[j] = tempCenter;
			}
	return centroid;
}

//Compute the Histogram Intersection
int Image::histIntersect(int histo1[], int histo2[], int num)
{
	int sum = 0;
    for(int i = 0; i < num; i++)
    {
