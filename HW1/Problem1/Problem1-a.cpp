...
int main(int argc, char* argv[])
{
	...
	char dog_demosaic[50] = "Images/Dog_demosaic.raw";
	...
	Image dogDemosaic(dog_width, dog_height, 3, dog_demosaic);

	//Initialize Objects
	dogInput.initialize();
	dogOrigin.initialize();
	dogDemosaic.initialize();

	//Read the Input Files
	dogInput.readFile();
	dogOrigin.readFile();

	//Do the Jobs of Demosaicing Bilinear
	dogInput.imgDemosaicBil(dogDemosaic);

	//Compute the PSNR value
	dogDemosaic.imgPSNR(dogOrigin);

	//Write Results in Files
	dogDemosaic.writeFile();
}